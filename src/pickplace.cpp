...
{
  ros::init(argc, argv, "pick_place_bt");
  ros::NodeHandle nh;
  ...
  //PickPlaceNodes::WorldModel worldmodel(nh);  
  //PickPlaceNodes::RegisterNodes(factory, worldmodel);
  PickPlaceNodes::RegisterNodes(factory, nh);
...
*/
  ...
  if (! nh_private.getParam("log", log_enabled) )
  {
	...
	cout_logger.setEnabled(log_enabled);
  
  // Log trace of state changes in a file (~/.ros/bt_trace.fbl)
  ...
  // Publish status changes using ZeroMQ for monitoring in Groot editor
